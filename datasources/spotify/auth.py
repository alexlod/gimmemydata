...
# class SpotifyAuth(OAuth2):
#     def __init__(self):
#         super().__init__(
#             client_id=Config().get_param('SPOTIFY_CLIENT_ID'),
#             client_secret=Config().get_param('SPOTIFY_CLIENT_SECRET'),
#             token_url='https://accounts.spotify.com/api/token',
#             redirect_uri='https://irregular-expressions.com'
#         )
...
#     def update_tokens_in_db(self, access_token, refresh_token, expiration_timestamp):
#         conn = self.db_client.connection
#         cur = conn.cursor()
#         cur.execute(
#             """
...
#     def get_valid_access_token(self):
#         access_token, refresh_token, expiration_timestamp = self.get_tokens_from_db()
#         if time.time() > expiration_timestamp:
#             new_tokens = self.get_access_token(refresh_token=refresh_token)
#             access_token = new_tokens['access_token']
#             refresh_token = new_tokens['refresh_token']
#             expiration_timestamp = new_tokens['expires_at']
#             self.update_tokens_in_db(access_token, refresh_token, expiration_timestamp)
#         return access_token
